...
public class GPUSkinningSamplerEditor : Editor
{
	...
	private void OnGUI_Sampler(GPUSkinningSampler sampler)
	{
		...
		BeginBox();
		{
			...
			{
				EditorGUILayout.PropertyField(serializedObject.FindProperty("assetName"), assetNameContent);
					...
					EditorGUILayout.BeginVertical();
					{
						...
						for (int i = 0; i < no; i++)
						{
							...
							if (prop != null)
							{
								...
								if (j == 0)
								{
									EditorGUILayout.PropertyField(prop3, nullGuiContent);
									prop3.intValue = Mathf.Clamp(prop3.intValue, 0, 60);
								}
								if (j == 1)
								{
									EditorGUILayout.PropertyField(prop2, nullGuiContent);
								}
								if (j == 2)
								{
									GUI.enabled = sampler.IsAnimatorOrAnimation() && guiEnabled;
									EditorGUILayout.PropertyField(prop, nullGuiContent);
									GUI.enabled = true && guiEnabled;
								}
								if (j == 3)
								{
